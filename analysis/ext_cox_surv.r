...
#high_risk_surv_ext<- read_csv("C:/Users/qw/Documents/Github/prophy_effects_Sotro_Molnup/output/data/high_risk_cohort_inc60ddeath.csv") %>%
high_risk_surv_ext <- read_csv(here::here("output", "data", "high_risk_cohort_inc60ddeath.csv")) %>%
...
str(high_risk_surv_ext, list.len = ncol(high_risk_surv_ext), give.attr= F)

cat("#freq_single(high_risk_surv_ext$ethnicity_num)")
freq_single(high_risk_surv_ext$ethnicity_num)

cat("#high_risk_surv_ext$drug, high_risk_surv_ext$surv6m_event_num")
table(high_risk_surv_ext$drug, high_risk_surv_ext$surv6m_event_num)
cat("#high_risk_surv_ext$stp, high_risk_surv_ext$surv6m_event_num")
table(high_risk_surv_ext$stp, high_risk_surv_ext$surv6m_event_num)

options(scipen = 999)
#strata(region_num)
cat("#summary(cox_model0)")
cox_model0 <- coxph(Surv(surv6m_days, surv6m_event_num) ~ (drug), data = high_risk_surv_ext) 
summary(cox_model0)

cat("#1summary(cox_model_strata(region_num))")
cox_model_region<- coxph(Surv(surv6m_days, surv6m_event_num) ~ (drug)+ strata(region_num), data = high_risk_surv_ext) 
summary(cox_model_region)

cat("#2summary(cox_model_strata(stp))")
cox_model_stp <- coxph(Surv(surv6m_days, surv6m_event_num) ~ (drug)+ strata(stp), data = high_risk_surv_ext)
...
+ strata(region_num), data = high_risk_surv_ext)
...
+ high_risk_num + strata(region_num), data = high_risk_surv_ext)
...
cox_model1_age_sex_highrisk_vacc_imd_reg_eth_region <- coxph(Surv(surv6m_days, surv6m_event_num) ~ drug + age_treated + sex_num 
    + high_risk_num + covid_vacc_num + imd_num + ethnicity_num + ns(calendar_day, df = 4) + strata(region_num), data = high_risk_surv_ext) 
...
+ chronic_cardiac_disease + chronic_respiratory_disease + strata(region_num), data = high_risk_surv_ext)
...
+ strata(stp), data = high_risk_surv_ext)
...
+ high_risk_num + strata(stp), data = high_risk_surv_ext)
...
cox_model1_age_sex_highrisk_vacc_imd_reg_eth_stp <- coxph(Surv(surv6m_days, surv6m_event_num) ~ drug + age_treated + sex_num 
    + high_risk_num + covid_vacc_num + imd_num + ethnicity_num + ns(calendar_day, df = 4) + strata(stp), data = high_risk_surv_ext)
...
+ sex_num + high_risk_num + covid_vacc_num + imd_num + ethnicity_num + ns(calendar_day, df = 4) + bmi_cat_num + diabetes + hypertension 
+ chronic_cardiac_disease + chronic_respiratory_disease + strata(stp), data = high_risk_surv_ext)
...
write.csv(surv0_regn, here::here("output", "tables", "table_ext_cox_surv0_regn_6m.csv"))
write.csv(surv0_stp, here::here("output", "tables", "table_ext_cox_surv0_stp_6m.csv"))
write.csv(high_risk_surv_ext, here::here("output", "data", "ext_high_risk_surv_data.csv"))
