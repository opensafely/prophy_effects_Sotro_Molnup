...
#high_risk_surv_ext2<- read_csv("C:/Users/qw/Documents/Github/prophy_effects_Sotro_Molnup/output/data/high_risk_cohort_inc60ddeath.csv") %>%
high_risk_surv_ext2 <- read_csv(here::here("output", "data", "high_risk_cohort_inc60ddeath.csv")) %>%
...
str(high_risk_surv_ext2, list.len = ncol(high_risk_surv_ext2), give.attr= F)

cat("#freq_single(high_risk_surv_ext2$ethnicity_num)")
freq_single(high_risk_surv_ext2$ethnicity_num)

cat("#high_risk_surv_ext2$drug, high_risk_surv_ext2$surv24m_event_num")
table(high_risk_surv_ext2$drug, high_risk_surv_ext2$surv24m_event_num)
cat("#high_risk_surv_ext2$stp, high_risk_surv_ext2$surv24m_event_num")
table(high_risk_surv_ext2$stp, high_risk_surv_ext2$surv24m_event_num)

options(scipen = 999)
#strata(region_num)
cat("#summary(cox_model0)")
cox_model0 <- coxph(Surv(surv24m_days, surv24m_event_num) ~ (drug), data = high_risk_surv_ext2) 
summary(cox_model0)

cat("#1summary(cox_model_strata(region_num))")
cox_model_region<- coxph(Surv(surv24m_days, surv24m_event_num) ~ (drug)+ strata(region_num), data = high_risk_surv_ext2)
summary(cox_model_region)

cat("#2summary(cox_model_strata(stp))")
cox_model_stp <- coxph(Surv(surv24m_days, surv24m_event_num) ~ (drug)+ strata(stp), data = high_risk_surv_ext2)
...
+ strata(region_num), data = high_risk_surv_ext2)
...
+ high_risk_num + strata(region_num), data = high_risk_surv_ext2)
...
cox_model1_age_sex_highrisk_vacc_imd_reg_eth_region <- coxph(Surv(surv24m_days, surv24m_event_num) ~ drug + age_treated + sex_num 
    + high_risk_num + covid_vacc_num + imd_num + ethnicity_num + ns(calendar_day, df = 4) + strata(region_num), data = high_risk_surv_ext2) 
...
+ chronic_cardiac_disease + chronic_respiratory_disease + strata(region_num), data = high_risk_surv_ext2)
...
+ strata(stp), data = high_risk_surv_ext2)
...
+ high_risk_num + strata(stp), data = high_risk_surv_ext2)
...
cox_model1_age_sex_highrisk_vacc_imd_reg_eth_stp <- coxph(Surv(surv24m_days, surv24m_event_num) ~ drug + age_treated + sex_num 
    + high_risk_num + covid_vacc_num + imd_num + ethnicity_num + ns(calendar_day, df = 4) + strata(stp), data = high_risk_surv_ext2)
...
+ sex_num + high_risk_num + covid_vacc_num + imd_num + ethnicity_num + ns(calendar_day, df = 4) + bmi_cat_num + diabetes + hypertension 
+ chronic_cardiac_disease + chronic_respiratory_disease + strata(stp), data = high_risk_surv_ext2) 
...
cat("#freq_single(high_risk_surv_ext2$surv24m_event_num)")
freq_single(high_risk_surv_ext2$surv24m_event_num)
cat("#freq_single(high_risk_surv_ext2$surv24m_event_num)")
summary(as.numeric(high_risk_surv_ext2$surv24m_event_num),na.rm=T)
cat("#sum(is.na(surv24m_event_num)):")
sum(is.na(high_risk_surv_ext2$surv24m_event_num))
cat("#sum(!is.na(surv24m_event_num)):")
sum(!is.na(high_risk_surv_ext2$surv24m_event_num))

cat("#freq_single(high_risk_surv_ext2$surv24m_days)")
summary(as.numeric(high_risk_surv_ext2$surv24m_days),na.rm=T)


# Save dataset(s) ----
write.csv(surv2_regn, here::here("output", "tables", "table_ext_cox_surv2_regn_24m.csv"))
write.csv(surv2_stp, here::here("output", "tables", "table_ext_cox_surv2_stp_24m.csv"))
write.csv(high_risk_surv_ext2, here::here("output", "data", "ext_high_risk_surv2_data.csv"))
